  ...
  async (request: CallableRequest<RequestData>) => {
    ...
    try {
      ...
      const geminiRequest: GeminiRequest = {
        contents: {
          ...
          parts: [
            ...
            {
              text: "Extract academic transcript into json format with sample output [{'subject': 'Algebra 2', 'grade': 'A', 'type': 'Regular', 'year': '2020' }]",
            },
          ],
        },
        generationConfig: {
      ...
      try {
        ...
        const subjects = parsedData.map((item: any) => ({
          name: item.subject,
          grade: item.grade,
          level: item.type,
          year: item.year,
